...
T execute_spec_precision(int conditions[])
{
    ...
    if ( r <= 0.16) {
	    ...
	    conditions[0] = 0;
		if ( r <= ((x1*x1)/0.4  + (x2*x2)/4.9)) {
			conditions[1] = 1;
			res = (x1+x2)*(x1+x2) - x1*x2 ;
		} else {
		    conditions[1] = 0;
			res = (x1+x2)*sin((double) (pi*(x1+x2)));
		}
	}
...
int main(int argc, char** argv)
{
	...
	int N;
	sscanf(argv[1], "%d", &N);
	fp = fopen("mod-test3_profile.csv", "w+");
    ofstream fp_divergence_inputs;
	fp_divergence_inputs.open("mod-test3_divergence_inputs.csv", ios::out | ios::app);
